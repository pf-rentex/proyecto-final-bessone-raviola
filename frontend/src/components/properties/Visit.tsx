...
const Visit = ({onClose}: {onClose: () => void} ) => {
    ...
    const schedules: { time: string, period: string; }[] = [{time: '09:00', period: 'AM'}, {
        time: '10:30',
        period: 'AM'
    }, {time: '16:00', period: 'PM'}, {time: '17:30', period: 'PM'}];
    ...
    return (
        <Modal title="Organizar visita"
               action={{text: 'Enviar solicitud', callback: () => {}}}
               onClose={onClose}
               content={
                   <>
                       <div className="px-3 py-6 flex items-center border-b">
                           <div className="flex-1 flex-col  space-y-6 mx-2">
                               <p><b>Ciudad:</b> San Francisco</p>
                               <p><b>Domicilio:</b> General Paz</p>
                       ...
                       <div className="px-4 flex flex-col md:flex-row justify-center">
                           ...
                           <div className="flex flex-col mx-3">
                               ...
                               <div className="my-6 px-3">
                                   {schedules.map((schedule, index) => {
                                       ...
                                       const classes = `rounded px-3 py-1 m-2 cursor-pointer ${isSelected ? 'bg-blue-800 shadow-inner-xl' : 'bg-gray-200 shadow-md'}`;

                                           return (
                                               <div key={index + time} className={classes} onClick={() => selectSchedule(schedule)}>
                                                   <div className={`text-lg text-center ${isSelected ? 'text-gray-100' : ''}`}>{time}
                                                       <b>{period}</b></div>
                                               </div>
                                           );
                                       }
                           ...
                           <div className="flex-1 mx-3 ">
                                 <textarea name="" id="" className="bg-gray-200 p-4 rounded w-full h-3/6 max-h-48	"
