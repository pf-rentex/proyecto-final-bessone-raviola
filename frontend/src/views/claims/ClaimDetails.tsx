    ...
    const getIcon = (category: string) => {
        switch (category) {
            case ClaimCategory.electricity:
                return <BsLightningFill className="text-white w-5 h-5 md:w-14 md:h-14" />;
            case ClaimCategory.plumbing:
                return <BsFillDropletFill className="text-white w-5 h-5 md:w-14 md:h-14" />;
            case ClaimCategory.infrastructure:
                return <BsFillGearFill className="text-white w-5 h-5 md:w-14 md:h-14" />;
        }
    };
            ...
            {isLoading ? (
                ...
                <div className="bg-gradient-to-b from-bg-gradient-10 rounded-xl px-10 py-14">
                    <div className="flex flex-col mb-10">
                        <div className="flex flex-col md:flex-row items-center space-y-5 md:space-y-0 md:space-x-5">
                            <div className="rounded-full bg-alt p-5">
                                {editing ? getIcon(claimData.category) : getIcon(claim.category)}
                            </div>
                            ...
                            <div className="flex flex-col space-y-3 w-full xl:w-4/12 text-white">
                                <p>
                                    ...
                                    <select
                                        ...
                                        value={!editing ? claim.category : claimData.category}
                                        name="category"
                                        onChange={(e) => {
                                            handleChange(e);
                                        }}
                                    >
                                        <option value={ClaimCategory.electricity}>Electricidad</option>
                                        <option value={ClaimCategory.plumbing}>Plomería</option>
                                        <option value={ClaimCategory.infrastructure}>Infraestructura</option>
                                    </select>
                                </p>
                                <p>
                                    Estado:{' '}
                                    <span className="text-yellow-300 font-bold">
                                        <select
                                            ...
                                            aria-label="Default select example"
                                            value={!editing ? claim.status : claimData.status}
                                            name="status"
                                            onChange={(e) => {
                                                handleChange(e);
                                            }}
                                        >
                                            <option value={ClaimStatus.inProgress}>En progreso</option>
                                            <option value={ClaimStatus.pending}>Pendiente</option>
                                            <option value={ClaimStatus.cancelled}>Cancelado</option>
                                            <option value={ClaimStatus.addressed}>Atendido</option>
                                        </select>
                                    </span>
                                </p>
                                <p>
                                    ...
                                    <input
                                        ...
                                        value={formatDate(new Date(!editing ? claim.createdAt : claimData.createdAt))}
                                        disabled={!editing}
                                        name="createdAt"
                                        onChange={(e) => {
                                            handleChange(e);
                                ...
                                <p>
                                    ...
                                    <input
                                        type="date"
                                        className={`text-white rounded ${inputBG}`}
                                        value={formatDate(new Date(!editing ? claim.dateVisit : claimData.dateVisit))}
                                        disabled={!editing}
                                        name="dateVisit"
                                        onChange={(e) => {
                                            handleChange(e);
                                ...
                                <div className="w-full lg:w-8/12">
                                    <CustomButton text="Reprogramar visita" callback={() => {}} color="alt">
                                        <BiCalendarEdit />
                                    </CustomButton>
                                </div>
                            </div>
                    ...
                    <div className="flex flex-col md:px-28 mb-10">
                        <h1 className="text-white text-3xl font-bold mb-3">Descripción</h1>
                    ...
                    <div className="flex flex-col sm:flex-row w-full md:w-6/12 sm:space-x-5 sm:px-20 md:px-28">
                        <CustomButton text="Resolver" callback={() => {}} color="alt">
